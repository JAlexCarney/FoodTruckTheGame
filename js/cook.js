...
Paw.prototype.update = function(){
	...
	if(this.isLeft){
		...
		// only let the right paw move left until it has crossed the left Paw.
		else if(game.input.keyboard.isDown(Phaser.Keyboard.D) && rightPaw.x > (this.x + 64)){
			this.body.velocity.x = 750;
	...
	// right paw
		...
		// only let the right paw move left until it has crossed the left Paw.
		if(game.input.keyboard.isDown(Phaser.Keyboard.J) && leftPaw.x < (this.x - 64)){
			this.body.velocity.x = -750;
			//console.log('moving Left!');
